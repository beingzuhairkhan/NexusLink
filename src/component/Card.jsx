...
const Card = () => {
    ...
    return (
        <div className="flex flex-col items-center justify-center mt-10 px-4">
            <div className="space-y-4 w-full max-w-2xl  hover:text-black ">
                {links.map((link, index) => (
                    <a
                        ...
                        className=" flex items-center justify-between rounded-lg p-2 shadow-md w-full h-16 hover:bg-gray-100 transition-all duration-300"
                    >
                        <img src={link.image} alt={link.title} className="w-12 h-12 rounded-md" />
                        <span className="text-sm font-medium  text-center flex-grow px-2 hover:text-black">{link.title}</span>
                        <FiCopy
                            onClick={(e) => {
                                e.stopPropagation(); 
